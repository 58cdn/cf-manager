...
- 双后端支持通过 `/admin/unlock/{UNLOCK_KEY}` 清除当前 IP 的失败与锁定记录，解锁后仍需 API Secret 登录。密钥由环境变量提供；Pages 部署 CI 未配置时自动生成，并明文输出一次供管理员保存，随后脱敏并写入部署 Secret。
- 双后端新增 API Secret 按 IP 失败锁定：默认连续错误 3 次锁 5 分钟，解锁后再错 1 次锁 10 分钟，成功认证重置。开关、首次次数与两阶段时长支持环境变量配置，状态保存到 SQLite / D1。
- 锁定返回 HTTP 429、`AUTH_LOCKED` 与 `Retry-After`，锁定期间正确凭据也不放行且请求不延长锁定；登录页显示剩余秒数并防止重复提交。
- Docker 默认使用直连 IP，反向代理需显式配置可信地址；Worker 使用 Cloudflare 注入的客户端 IP。Worker 升级需执行 `0010_auth_lockouts.sql` 迁移。
