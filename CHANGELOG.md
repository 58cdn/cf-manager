...
- 保留原管理界面为默认主题 `default`，新增「青岚 / Verdant」主题，优化导航、登录、仪表盘统计、卡片、表格与工具栏，两种主题均支持独立的深浅模式。
- 登录页、桌面顶部和移动导航提供即时主题切换；共享主题配置同步更新弹窗、通知和 discrete API，不重新挂载业务页面。
- 使用 `cf-manager.ui-theme` 按同一浏览器、同一源保存主题，兼容已有 `darkMode`；存储不可用时支持当前会话内切换并提示无法保存。
- 补充移动导航键盘操作、焦点返回与双语外观文案，保持 Docker `/` 和 Worker `/admin/` 前端路径兼容，不新增生产依赖或后端接口。
- 补充 Worker Cron Triggers 管理：双端 schedules API 校验五字段表达式并规范化空白，显式 `crons: []` 可清理旧触发器，未声明 `crons` 的 Catalog 部署保留现有配置。
